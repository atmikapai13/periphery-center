...
/**
 * Pre-calculates deterministic timing for smooth transitions (no randomization)
 */
export function calculateRevealTimings(
  textPositions: Set<string>,
  ...
  // Text positions: staggered reveal from left to right, top to bottom
  const textArray = Array.from(textPositions);
  textArray.forEach((pos, index) => {
    const [row, col] = pos.split(',').map(Number);

    // Systematic timing based on position (left-to-right, top-to-bottom)
    const baseDelay = (col * 20) + (row * 5); // Deterministic, no randomness
    const finalDelay = Math.min(baseDelay, totalDuration * 0.7);

    timings.set(pos, finalDelay);
  });

  // Background positions: earlier fade start
  backgroundPositions.forEach(pos => {
    const [row, col] = pos.split(',').map(Number);

    // Background fades start earlier and are based on distance from center
    const centerRow = 30; // Approximate center
    const centerCol = 50;
    const distance = Math.sqrt((row - centerRow) ** 2 + (col - centerCol) ** 2);
    const fadeDelay = distance * 10; // Deterministic fade based on distance

    timings.set(pos, fadeDelay);
  });
